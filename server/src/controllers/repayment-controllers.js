...
export async function getRepayments(req, res) {
  try {
    const [rows] = await db.query("SELECT * FROM loan_repayment");
...
export async function getRepaymentsByLoan(req, res) {
  try {
    const [rows] = await db.query("SELECT * FROM loan_repayment WHERE loan_id=?", [
      req.params.loan_id,
    ]);
    res.json({ success: true, data: rows });
  } catch (err) {
...
export async function createRepayment(req, res) {
  ...
  try {
    ...
    const [lastRepayment] = await db.query(
      `SELECT balance FROM loan_repayment 
...
export async function deleteRepayment(req, res) {
  try {
    await db.query("DELETE FROM loan_repayment WHERE repayment_id=?", [
      req.params.id,
    ]);
