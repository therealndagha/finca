...
export async function getAllLoans(req, res) {
  try {
    const [rows] = await db.query("SELECT * FROM loan_product ORDER BY product_id ASC");
...
export async function getLoanById(req, res) {
  const { id } = req.params;
  try {
    const [rows] = await db.query("SELECT * FROM loan_product WHERE product_id = ?", [id]);
...
export async function createLoan(req, res) {
  ...
  try {
    ...
    const [rows] = await db.query("SELECT * FROM loan_product WHERE product_id = ?", [result.insertId]);
...
export async function updateLoan(req, res) {
  ...
  try {
    const [existing] = await db.query("SELECT * FROM loan_product WHERE product_id = ?", [id]);
    if (existing.length === 0) return res.status(404).json({ success: false, message: "Loan product not found" });

    await db.query(
      "UPDATE loan_product SET product_name=?, interest_rate=?, max_amount=?, min_amount=?, tenure_months=?, description=? WHERE product_id=?",
...
export async function deleteLoan(req, res) {
  ...
  try {
    ...
    await db.query("DELETE FROM loan_product WHERE product_id = ?", [id]);
